...
- (void)addHeaderWithTarget:(id)target action:(SEL)action
{
    // 1.创建新的header
    if (!self.mj_header) {
        PreciousMetalCustomHeader *header = [PreciousMetalCustomHeader headerWithRefreshingTarget:target refreshingAction:action];
    ...
    // 2.设置目标和回调方法
    //    self.mj_header.beginRefreshingTaget = target;
    //    self.mj_header.beginRefreshingAction = action;
}

- (void)addFooterWithTarget:(id)target action:(SEL)action
{//[MJRefreshAutoNormalFooter footerWithRefreshingTarget:self refreshingAction:@selector(footRefresh)];
    ...
    // 2.设置目标和回调方法
    //    self.mj_footer.beginRefreshingTaget = target;
    //    self.mj_footer.beginRefreshingAction = action;
}
